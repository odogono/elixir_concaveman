if Mix.env() == :dev do
  defmodule Concaveman.RunLive do
    ...
    def mount(_params, _session, socket) do
      ...
      {:ok, assign(socket, svg_content: nil, concavity_value: 2.0, length_value: 0.0)}
    end
    ...
    def render(assigns) do
      ...
      <div class="flex flex-col items-center justify-center min-h-screen">
        <div id="dropzone" phx-hook="DropZone" class="w-[1024px] h-[1024px] border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center mb-4">
          <%= if @svg_content do %>
    ...
    def handle_event("slider_changed", %{"id" => id, "value" => value}, socket) do
      ...
      # Set to true to render points
      svg = generate_svg(geojson_data, hull, concavity, length, true)
    ...
    defp generate_svg(geojson_data, hull, concavity, length, render_points)
      ...
      path_data =
        ...
        |> Enum.map_join(" ", fn {x, y} ->
          ...
          # Flip Y-axis
          sy = height - ((y - min_y) * scale + padding) - height / 4
          "#{sx},#{sy}"
        end)
